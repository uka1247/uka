...
補足の冒頭には「補足になりますが、」などの定型句は使ってください。
"""}
        ]
        ...
        extra_match = re.search(r"🔴 視点をずらした立場：.*?(?=\n\n|$)(.*)", result, flags=re.DOTALL)
        if extra_match and extra_match.group(1).strip():
    　　　　　st.markdown(f'<div class="box extra">{extra_match.group(1).strip()}</div>', unsafe_allow_html=True)
